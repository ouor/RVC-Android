...
private const val TAG = "Rvc.QnnRmvRunner"
...
// Sibling of QnnSynthRunner / QnnHubertRunner. Spawns
// librvc_rmvpe_runner.so as a separate child process so RMVPE gets
// its own fastrpc PD on the DSP. Each infer() pumps fp32 audio +
// threshold in and reads back fp32 pitchf — no dtype conversion on
// either side.
class QnnRmvpeRunner(
    ctx: Context,
    binPath: String,
) : Closeable {
    ...
    init {
        ...
        val runner = File(nativeLibDir, "librvc_rmvpe_runner.so")
        require(runner.exists()) {
            "rmvpe runner binary not found: ${runner.absolutePath} — extractNativeLibs may be off"
        }
        ...
        process = pb.start()
        // Body is small (~120 KiB request, ~1 KiB response) so 64 KiB
        // buffers are plenty; one flush still coalesces into a single
        // pipe write.
        out = DataOutputStream(BufferedOutputStream(process.outputStream, 64 shl 10))
        inp = DataInputStream(BufferedInputStream(process.inputStream, 64 shl 10))
        ...
        Thread {
            ...
            isDaemon = true
            name = "QnnRmvpeRunner-stderr"
            start()
        }

        val magic = readIntLE(inp)
        require(magic == MAGIC_REDY) {
            "rmvpe runner did not send READY (got 0x${magic.toUInt().toString(16)})"
        }
        Log.i(TAG, "runner READY")
    }

    data class RmvpeOutput(val pitchf: FloatArray, val numFrames: Int)

    fun infer(audio: FloatArray, threshold: Float): RmvpeOutput {
        writeIntLE(out, MAGIC_INFR)
        writeIntLE(out, audio.size)
        writeFloatArray(out, audio)
        writeFloatLE(out, threshold)
        out.flush()
        ...
        val pitchf = readFloatArrayWire(inp)
        val errMsg = readString(inp)
        if (status != 0) error("rmvpe runner inference failed (status=$status): $errMsg")
        return RmvpeOutput(pitchf, numFrames)
    }
...
private fun writeFloatLE(out: DataOutputStream, v: Float) {
    writeIntLE(out, java.lang.Float.floatToRawIntBits(v))
}
...
private fun readIntLE(inp: DataInputStream): Int {
    ...
    val b2 = inp.read()
    val b3 = inp.read()
    if (b0 < 0 || b1 < 0 || b2 < 0 || b3 < 0) error("unexpected EOF on rmvpe runner stream")
